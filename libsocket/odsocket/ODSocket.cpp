...
ODSocket& ODSocket::operator = (const ODSocket & s)
{
	m_sock = s.m_sock;
	return (*this);
}

bool ODSocket::operator == (SOCKET s)
{
	return m_sock == s;
}
bool ODSocket::operator == (const ODSocket & s)
{
	return m_sock == s.m_sock;
}
...
SOCKET ODSocket::getSocket() const
{
	return this->m_sock;
}
...
int ODSocket::Ioctl(long cmd, u_long* argp)
{
#ifdef WIN32
	return (ioctlsocket(m_sock,cmd,argp));
#else
	return (ioctl(m_sock,cmd,argp));
#endif
}

timeval ODSocket::buildTimeval(long tv_set, long tv_uset)
{
	timeval tm;
	tm.tv_sec = tv_set;
	tm.tv_usec = tv_uset;
